...
class OsteoMAENet(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, self.num_patches + 1, decoder_emb_size)
        )

        decoder_layer = nn.TransformerEncoderLayer(
            d_model=decoder_emb_size,
            nhead=8,
            dim_feedforward=decoder_emb_size * 4,
            activation="gelu",
            batch_first=True,
            norm_first=True,
        )
        ...
        self.supcon_head = nn.Sequential(
            nn.Linear(emb_size, 128), nn.GELU(), nn.Linear(128, 128)
        )
    ...
    def forward(self, x, mask=True):
        ...
        supcon_embeds = nn.functional.normalize(
            supcon_embeds, dim=1
