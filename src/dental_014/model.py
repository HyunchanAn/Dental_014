...
class MaskAttentionResNet(nn.Module):
    ...
    def __init__(self, num_classes=3, pretrained=True, unet_weights_path=None):
        ...
        if unet_weights_path and os.path.exists(unet_weights_path):
            self.unet.load_state_dict(torch.load(unet_weights_path, map_location='cpu', weights_only=True))
        ...
        self.resnet.conv1 = nn.Conv2d(4, original_conv.out_channels, 
...
def get_model(num_classes=3, pretrained=True, unet_weights_path=None):
    ...
    return MaskAttentionResNet(num_classes=num_classes, pretrained=pretrained, unet_weights_path=unet_weights_path)
