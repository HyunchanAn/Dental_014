...
class OrdinalFocalLoss(nn.Module):
    ...
    def __init__(self, num_classes=3, gamma=2.0, alpha=None):
        ...
        self.register_buffer(
            "distance_matrix", self._create_distance_matrix(num_classes)
        )
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        mask_pos_pairs = torch.where(
            mask_pos_pairs < 1e-6, 1.0, mask_pos_pairs
        )  # division by zero 방지
...
class OsteoCompositeLoss(nn.Module):
    ...
    def __init__(self, recon_weight=1.0, ordinal_weight=1.0, supcon_weight=0.5):
        ...
        self.ordinal_loss = OrdinalFocalLoss(
            num_classes=3, gamma=2.0, alpha=class_weights
        )
