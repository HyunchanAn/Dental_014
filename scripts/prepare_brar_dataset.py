...
def prepare_brar_dataset(source_dir, dest_dir, split_ratio=0.8, seed=42):
    ...
    for level, c_class in level_map.items():
        ...
        images = [f for f in os.listdir(level_dir) if f.lower().endswith(('.jpg', '.png'))]
        ...
        print(f"{level} -> {c_class}: {len(train_images)} train, {len(test_images)} test")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source", type=str, default=r"C:\Users\chema\Github\Dental_014\data\BRAR_dataset", help="Path to raw BRAR dataset")
    parser.add_argument("--dest", type=str, default=r"C:\Users\chema\Github\Dental_014\data\clean_cropped_ds", help="Destination path")
