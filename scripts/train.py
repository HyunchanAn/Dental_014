...
def parse_args():
    ...
    parser.add_argument(
        "--save_dir", type=str, default="weights", help="Directory to save weights"
    )
    parser.add_argument(
...
def train(args):
    ...
    criterion = OsteoCompositeLoss(
        recon_weight=1.0, ordinal_weight=1.0, supcon_weight=0.5
    ).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for inputs, labels in pbar:
            ...
            loss, loss_recon, loss_ordinal, loss_supcon = criterion(
                pred_pixel, target_pixel, class_logits, supcon_embeds, labels
            )
        ...
        with torch.no_grad():
            for inputs, labels in tqdm(
                ...
                loss, loss_recon, loss_ordinal, loss_supcon = criterion(
                    None, None, class_logits, supcon_embeds, labels
                )
